...
echo "🔧 CORREÇÃO V3 — LEVEL + FIGURINHAS"
...
# ============================================================
# 1. CORRIGIR handleLevel
# ============================================================

p = Path("dados/src/features/kyaraSpecialCommands.js")
s = p.read_text()

# Garante que handleLevel receba prefix.
s, n = re.subn(
    r"async function handleLevel\s*\(\{\s*sender,\s*pushname(?:,\s*prefix)?",
    "async function handleLevel({ sender, pushname, prefix = '/'",
    s,
    count=1
)

if n:
    print("✅ handleLevel: prefix corrigido.")
else:
    # Caso a assinatura esteja em outra formatação.
    pos = s.find("async function handleLevel")
    if pos != -1:
        end = s.find("})", pos)
        trecho = s[pos:end+2]
        if "prefix" not in trecho:
            novo = trecho.replace(
                "{ sender, pushname",
                "{ sender, pushname, prefix = '/'"
            )
            s = s.replace(trecho, novo, 1)
            print("✅ handleLevel: prefix inserido.")
        else:
            print("ℹ️ handleLevel já possui prefix.")
    else:
        print("⚠️ handleLevel não localizado.")

# Corrige qualquer uso potencialmente inseguro de prefix.
s = s.replace(
    "Digite *${prefix}levelinfo*",
    "Digite *${prefix || '/'}levelinfo*"
)
...
# ============================================================
# 2. NOVO SISTEMA DE FIGURINHAS
# ============================================================

p = Path("dados/src/features/themeStickers.js")

p.write_text(r'''import axios from 'axios';
...
print("✅ themeStickers.js V3 instalado.")
PY
...
echo
echo "=== VERIFICANDO LEVEL ==="
grep -n "async function handleLevel" \
  dados/src/features/kyaraSpecialCommands.js | head -3

echo
echo "=== VERIFICANDO LEVELINFO ==="
grep -n "levelinfo" \
  dados/src/features/kyaraSpecialCommands.js | head -10

echo
echo "=== VERIFICANDO HISTÓRICO DE FIGURINHAS ==="
grep -n "sticker-history\|used.has\|Math.random" \
  dados/src/features/themeStickers.js | head -20

echo
echo "============================================"
echo "✅ CORREÇÃO V3 CONCLUÍDA"
echo "============================================"
echo
echo "Agora:"
echo "npm start"
