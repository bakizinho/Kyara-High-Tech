...
const TEMP_DIR = path.join(process.cwd(), 'temp_kyara');
...
async function displayHeader() {
  const header = [
    `${colors.bold}🚀 Kyara - Atualizador${colors.reset}`,
...
async function checkRequirements() {
  ...
  try {
    ...
    printWarning('⚠️ Git não encontrado! É necessário para atualizar o Kyara.');
    if (isWindows) {
...
async function downloadUpdate() {
  printMessage('📥 Baixando a versão mais recente do Kyara...');
...
async function main() {
  ...
  try {
    ...
    const response = await fetch(`https://api.github.com/repos/${config.autor}/kyara/commits?per_page=1`, {
