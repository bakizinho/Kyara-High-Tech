...
async function loadModules() {
    ...
    modulesPromise = (async () => {
        ...
        // Cria um objeto mutável para módulos ESM, evitando
        // tentar modificar diretamente o namespace retornado por import().
        // YouTube usa exports nomeados: search, mp3 e mp4.
        // Copiamos explicitamente para um objeto mutável.
        modules.youtube = {
            search: youtubeMod.search,
            info: youtubeMod.info,
            mp3: youtubeMod.mp3,
            mp4: youtubeMod.mp4,
            ytmp3: youtubeMod.ytmp3 ?? youtubeMod.mp3
        };

        if (typeof modules.youtube.search !== 'function') {
            console.warn('[EXPORTS] YouTube search function not found');
        }

        if (typeof modules.youtube.mp3 !== 'function') {
            console.warn('[EXPORTS] YouTube.mp3 not available');
        }

        if (typeof modules.youtube.mp4 !== 'function') {
            console.warn('[EXPORTS] YouTube.mp4 not available');
        }
