...
class AutoRestarter {
    constructor() {
        ...
        this.pidFile = path.join(__dirname, '../../../kyara.pid');
    ...
    async restartProcess() {
        try {
            ...
            const child = spawn(process.execPath, [...nodeArgs, ...args], {
                ...
                env: {
                    ...process.env,
                    KYARA_RESTARTED: 'true',
                    KYARA_RESTART_COUNT: this.restartCount.toString()
                }
            });
    ...
    async start() {
        try {
            ...
            if (process.env.KYARA_RESTARTED === 'true') {
                await this.logEvent('restart_success', {
                    previousRestartCount: process.env.KYARA_RESTART_COUNT || 'unknown'
                });
            }
        } catch (error) {
