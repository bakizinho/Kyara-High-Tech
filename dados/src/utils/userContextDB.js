...
class UserContextDB {
  ...
  /**
   * Atualiza o relacionamento com Kyara
   */
  updateRelationship(userId, campo, valor) {
    const context = this.getUserContext(userId);
    
    if (context.relacionamento_kyara.hasOwnProperty(campo)) {
      context.relacionamento_kyara[campo] = valor;
      context.ultima_atualizacao = getBrazilDateTime();
      this.saveDatabase();
    }
  }
  ...
  addSpecialMemory(userId, memoria) {
    ...
    context.relacionamento_kyara.memorias_especiais.push(novaMemoria);
    
    // Manter apenas as 30 memórias mais especiais
    if (context.relacionamento_kyara.memorias_especiais.length > 30) {
      context.relacionamento_kyara.memorias_especiais = 
        context.relacionamento_kyara.memorias_especiais.slice(-30);
    }
  ...
  updateMemory(userId, tipo, valorAntigo, valorNovo) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        const indexMemoria = context.relacionamento_kyara.memorias_especiais.findIndex(
          m => m.texto === valorAntigo
        );
        if (indexMemoria !== -1) {
          context.relacionamento_kyara.memorias_especiais[indexMemoria].texto = valorNovo;
          context.relacionamento_kyara.memorias_especiais[indexMemoria].data = getBrazilDateTime();
          atualizado = true;
        }
        break;
  ...
  deleteMemory(userId, tipo, valor) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        ...
        if (indexMemoria !== -1) {
          context.relacionamento_kyara.memorias_especiais.splice(indexMemoria, 1);
          removido = true;
        }
        break;
  ...
  getUserContextSummary(userId) {
    ...
    const summary = {
      ...
      nivel_intimidade: context.relacionamento_kyara.nivel_intimidade,
      ...
      memorias_especiais: context.relacionamento_kyara.memorias_especiais.slice(-5).map(m => m.texto).join('\n- ') || 'Nenhuma'
    };
